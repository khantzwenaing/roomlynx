...
import { calculateExtraPersonCharge, getGasSettings } from "@/services/settingsService";
...
export const calculateAmountDue = async (room: Room, customer: Customer | null): Promise<{
  totalStay: number;
  deposit: number;
  extraPersonCharge: number;
  amountDue: number;
}> => {
  if (!customer) return { totalStay: 0, deposit: 0, extraPersonCharge: 0, amountDue: 0 };
...
export const calculateExtraPersonsCharge = async (customer: Customer): Promise<number> => {
